...
// Map Slack file IDs to corresponding OpenAI file information.
db.exec(`
    CREATE TABLE IF NOT EXISTS files (
        slack_file_id TEXT PRIMARY KEY,
        openai_file_id TEXT,
        openai_file_type TEXT
    ) STRICT`
);
...
app.event('app_mention', async ({ event, client, logger }) => {
    ...
    try {
        ...
        const get_replies = client.conversations.replies({
            channel,
            ts: thread_ts || ts,    // Reply to thread.
        });

        const create_file = Promise.all(
            (files ?? []).map(async (meta) => {
                const fetched = await fetch(meta.url_private_download, { headers: { Authorization: `Bearer ${process.env.SLACK_BOT_TOKEN}` } });
        ...
        const post_message = client.chat.postMessage({
            channel,
            thread_ts: thread_ts || ts, // Reply to thread.
